...
const SubscriptionDetails = () => {
    ...
    const handleGetMyActiveSubscriptionApi = async () => {
        ...
        try {
            ...
            setMyActiveSubscriptionDetails({});
        } finally {
            setIsLoading(false);
        }
        ...
        <div className="flex flex-col gap-6 animate-in fade-in duration-300">
            ...
            {isLoading ? (
                <div className="flex items-center justify-center min-h-[200px]">
                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-perple"></div>
                </div>
            ) : (!myActivesubscriptionDetails || Object.keys(myActivesubscriptionDetails).length === 0 || !myActivesubscriptionDetails.email) ? (
                <div className="bg-white p-8 rounded-xl border border-gray-150 shadow-sm flex flex-col items-center justify-center text-center gap-4 py-12">
                    <div className="bg-purple-50 p-4 rounded-full text-perple">
                        <ShoppingBag className="w-8 h-8" />
                    </div>
                    <div>
                        <h3 className="text-sm font-bold text-[#2F2B3D]">No subscription found</h3>
                        <p className="text-xs text-gray-500 mt-1 max-w-sm">
                            You do not have an active corporate subscription.
                        </p>
                    </div>
                    <button
                        onClick={() => navigate('/enroll-team')}
                        className="px-4 py-2 text-xs font-bold text-white bg-perple hover:opacity-90 rounded cursor-pointer transition-all duration-200"
                    >
                        Choose a Plan
                    </button>
                </div>
                            ...
                            <div>
                                <div className="flex justify-between items-start mb-3">
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            onClick={() => navigate('/enroll-team')}
                                            className="px-2.5 py-1 text-[10px] font-bold text-white bg-perple hover:opacity-90 rounded border border-transparent cursor-pointer transition-all duration-200"
                                        >
                                            Upgrade Plan
                                        </button>
                                        <button
