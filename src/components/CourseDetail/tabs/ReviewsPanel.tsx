    ...
    return (
        <div id="tabpanel-Reviews" role="tabpanel" aria-labelledby="tab-Reviews" className="mb-12">
            <SectionTitle
                ...
                title={t('courseDetail.studentReviews', 'Student Reviews')}
                subtitle={`${total.toLocaleString()} ${total === 1 ? t('courseDetail.reviewFromVerified', 'review from verified students') : t('courseDetail.reviewsFromVerified', 'reviews from verified students')}`}
            />
            ...
            <div className="bg-gradient-to-br from-indigo-50 to-purple-50 border border-indigo-100 rounded-2xl p-6 flex flex-col sm:flex-row items-center gap-6">
                ...
                <div className="text-center shrink-0">
                    ...
                    <p className="text-xs font-semibold text-indigo-600 mt-1 uppercase tracking-wide">{t('courseDetail.courseRating', 'Course Rating')}</p>
                </div>
                ...
                <div className="flex-1 text-center sm:text-left">
                    <p className="text-gray-600 text-sm leading-relaxed">
                        {t('courseDetail.basedOn', 'Based on')}{' '}
                        <span className="font-bold text-gray-900 text-base">{total.toLocaleString()}</span>{' '}
                        {total === 1 ? t('courseDetail.review', 'review') : t('courseDetail.reviews', 'reviews')}
                    </p>
                    {total === 0 ? (
                        <p className="mt-2 text-sm text-gray-400 italic">
                            {t('courseDetail.noReviewsYet', 'No reviews yet — be the first to share your experience!')}
                        </p>
                    ) : (
