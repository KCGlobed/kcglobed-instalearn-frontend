...
const Sidebar = ({ selectedSubcategories, setSelectedSubcategories }: any) => {
    const dispatch = useAppDispatch();
    const { category: categories, loading } = useAppSelector((state: any) => state.filterCategory);
    ...
    const toggleSubcategory = (id: number) => {
        setSelectedSubcategories((prev: number[]) => {
            if (prev.includes(id)) {
    ...
    return (
        <div className="w-full bg-white flex flex-col select-none">
            {/* Category Section */}
            <div className="border border-[#E9EAF0] p-5 pb-2 transition-all min-h-[100px]">
                <div
                    ...
                    <div className="space-y-1">
                        {categories && categories.map((cat: any, i: number) => {
                            ...
                            return (
                                <div key={cat.id}>
                                    <div
                                        className={`flex justify-between items-center cursor-pointer group ${i === 0 ? "py-2.5" : "py-3.5"}`}
                                        onClick={() => toggleCategory(cat.id)}
                                    >
                                        <div className={`flex items-center gap-3 ${isOpen ? "text-[#5624D0]" : "text-[#4E5566] group-hover:text-[#5624D0] transition-colors"}`}>
                                            <Icon
                                                className="w-[18px] h-[18px]"
                                                strokeWidth={i === 0 ? (isOpen ? 2 : 1.5) : 1.5}
                                            />
                                            <span className={`text-[14px] ${i === 0 || isOpen ? "font-medium" : ""}`}>
                                                {cat.name}
                                            </span>
                                        </div>
                                        {hasSubcategories ? (
                                    ...
                                    {i < categories.length - 1 && <div className="h-px bg-[#E9EAF0] my-1" />}
                                </div>
                            );
                        })}
                    </div>
            ...
            <div className="max-w-[1320px] mx-auto py-10 flex flex-col lg:flex-row gap-8">
                ...
                {isFilterOpen && (
                    <div className="w-full lg:w-[280px] xl:w-[312px] shrink-0 transition-all duration-300">
                        <Sidebar selectedSubcategories={selectedSubcategories} setSelectedSubcategories={setSelectedSubcategories} />
                    </div>
                )}
